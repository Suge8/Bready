...
const AnalysisReportModal: React.FC<AnalysisReportModalProps> = ({ analysis, onClose }) => {
  ...
  const getScoreBgColor = (score: number) => {
    if (score >= 80)
      return 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-900/30'
    if (score >= 60)
  ...
  return (
    <Modal isOpen onClose={onClose} size="xl" className="p-0 max-w-4xl max-h-[90vh] flex flex-col">
      <div className="flex items-center justify-between p-6 border-b border-[var(--bready-border)] flex-shrink-0">
        <h2 className="text-2xl font-bold text-[var(--bready-text)]">
          {t('prep.report.modalTitle')}
        </h2>
      </div>
          ...
          <Card className={`border-2 rounded-2xl ${getScoreBgColor(analysis.matchScore)}`}>
            <CardContent className="p-6 text-center">
              <div className="flex items-center justify-center mb-4">
                ...
                <h3 className="text-xl font-bold text-[var(--bready-text)]">
                  {t('prep.report.matchScore')}
                </h3>
              </div>
              ...
              <p className="text-[var(--bready-text-muted)]">
                {t('prep.report.fullScore', { count: 100 })}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="border border-[var(--bready-border)] shadow-lg rounded-2xl bg-[var(--bready-surface)] overflow-hidden">
            <CardHeader className="bg-zinc-50 dark:bg-zinc-900/30 p-6 border-b border-zinc-100 dark:border-zinc-900/40">
              <div className="flex items-center">
                ...
                <CardTitle className="text-xl font-bold text-[var(--bready-text)]">
                  {t('prep.report.suggestions')}
                </CardTitle>
              </div>
            </CardHeader>
            <CardContent className="p-6">
