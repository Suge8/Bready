  ...
  return (
    <div className={`${colors.bg} ${colors.border} border rounded-lg p-4 mb-4`}>
      <div className="flex items-start space-x-3">
        <div className={`${colors.icon} flex-shrink-0 mt-0.5`}>{config.icon}</div>

        <div className="flex-1 min-w-0">
          <h3 className={`${colors.title} font-medium text-sm`}>{config.title}</h3>
          <p className={`${colors.message} text-sm mt-1`}>{message}</p>
        </div>
