...
import { Target, TrendingUp, AlertTriangle, Lightbulb, Edit, Brain, RefreshCw, Loader2, Briefcase, Sparkles } from 'lucide-react'
    ...
    try {
      ...
      if (window.bready && window.bready.analyzePreparation) {
        ...
        analysisResultData = {
          ...
          analysis: {
            matchScore: 68,
            jobRequirements: ['3年以上游戏渠道运营经验', '熟悉WeGame/应用宝等主流渠道', '具备数据分析和用户增长能力', '有完整游戏发行经验优先', '热爱FPS游戏并有深度体验'],
      ...
      setPreparations(preparations.map(p => p.id === currentPreparation.id ? updatedPreparation : p))
      await onReloadData()
  ...
  const cardConfigs = [
    {
      ...
      title: '岗位需求',
      icon: Briefcase,
      gradient: isDarkMode ? 'from-blue-500/10 via-blue-500/5 to-transparent' : 'from-blue-50 to-white',
    ...
    },
    {
      ...
      title: '核心优势',
      icon: TrendingUp,
      gradient: isDarkMode ? 'from-emerald-500/10 via-emerald-500/5 to-transparent' : 'from-emerald-50 to-white',
    ...
    },
    {
      ...
      title: '改进空间',
      icon: AlertTriangle,
      gradient: isDarkMode ? 'from-orange-500/10 via-orange-500/5 to-transparent' : 'from-orange-50 to-white',
    ...
    },
    {
      ...
      title: '面试建议',
      icon: Lightbulb,
      gradient: isDarkMode ? 'from-violet-500/10 via-violet-500/5 to-transparent' : 'from-violet-50 to-white',
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[100] p-6 cursor-pointer" onClick={onClose}>
      ...
      >
        ...
        <div className="relative flex items-center justify-between px-6 py-4 flex-shrink-0 gap-3">
          {/* 左侧：标题 */}
          <h1 className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} truncate flex-shrink-0 max-w-[180px]`}>
            {currentPreparation.name}
          </h1>

          {/* 中间：分数气泡 */}
          {hasAnalysis && (
            <div className={`flex items-center gap-2.5 px-3 py-1.5 rounded-xl ${isDarkMode ? 'bg-zinc-800/80' : 'bg-gray-100/80'} flex-shrink-0`}>
              <Target className={`w-3.5 h-3.5 ${isDarkMode ? 'text-zinc-400' : 'text-gray-500'}`} />
              <span className={`text-xl font-bold ${getScoreColor(currentPreparation.analysis!.matchScore)}`}>{displayScore}</span>
              <span className={`text-xs ${isDarkMode ? 'text-zinc-500' : 'text-gray-400'}`}>/ 100</span>
              <div className={`w-16 h-1.5 rounded-full ${isDarkMode ? 'bg-zinc-700' : 'bg-gray-200'} overflow-hidden`}>
                <div className={`h-full rounded-full transition-all duration-700 ${getScoreBarColor(currentPreparation.analysis!.matchScore)}`} 
                  style={{ width: `${displayScore}%` }} />
              </div>
              <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${getScoreBarColor(currentPreparation.analysis!.matchScore)} text-white`}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <Button onClick={handleAnalyze} disabled={isAnalyzing} size="sm"
              ...
              {isAnalyzing ? (
                ...
                <><RefreshCw className="w-3 h-3 mr-1" />{currentPreparation.analysis ? t('prepEditor.actions.reanalyze') : t('prepEditor.actions.analyze')}</>
              )}
            </Button>
          </div>
        </div>
          ...
          {isAnalyzing ? (
            <div className="h-full min-h-[280px] flex items-center justify-center">
              <div className="flex flex-col items-center text-center">
                <div className="relative mb-5">
                  <div className={`w-16 h-16 rounded-2xl ${isDarkMode ? 'bg-zinc-800' : 'bg-gray-100'} flex items-center justify-center`}>
                    <Loader2 className={`w-8 h-8 ${isDarkMode ? 'text-white' : 'text-gray-700'} animate-spin`} />
                  </div>
                  <Sparkles className={`w-4 h-4 text-amber-400 absolute -top-1 -right-1 animate-pulse`} />
                </div>
                <h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'} mb-1.5`}>AI 正在分析中...</h3>
                <p className={`${isDarkMode ? 'text-zinc-500' : 'text-gray-500'} text-xs`}>正在根据岗位描述和简历生成分析报告</p>
              </div>
            </div>
          ) : hasAnalysis ? (
              ...
              {cardConfigs.map((config, index) => (
                <div key={config.key}
                  ...
                  <div className="flex items-center gap-2 mb-2.5 flex-shrink-0">
                    ...
                    <span className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>{config.title}</span>
                    <span className={`ml-auto text-[10px] px-1.5 py-0.5 rounded-full ${isDarkMode ? 'bg-zinc-800/80 text-zinc-400' : 'bg-white/80 text-gray-500'}`}>
                  ...
                  <ul className="space-y-2 flex-1 overflow-y-auto">
                    {config.data.length > 0 ? (
                      config.data.map((item: string, idx: number) => (
                        <li key={idx} className="flex items-start text-[13px] animate-fadeIn" style={{ animationDelay: `${idx * 50 + index * 60}ms` }}>
                          <div className={`w-1.5 h-1.5 rounded-full ${config.dotColor} mt-1.5 mr-2 flex-shrink-0`} />
                          <span className={`${isDarkMode ? 'text-zinc-300' : 'text-gray-700'} leading-relaxed`}>{item}</span>
                        </li>
                      ))
                    ) : (
                      <li className={`text-[13px] ${isDarkMode ? 'text-zinc-500' : 'text-gray-400'} italic`}>
                        点击"重新分析"生成内容
                      </li>
                    )}
                  </ul>
            ...
            <div className="h-full min-h-[250px] flex items-center justify-center">
              <div className="flex flex-col items-center text-center max-w-xs">
                <div className={`w-14 h-14 ${isDarkMode ? 'bg-zinc-800' : 'bg-gray-100'} rounded-xl flex items-center justify-center mb-3 relative`}>
                  <Brain className={`w-7 h-7 ${isDarkMode ? 'text-zinc-600' : 'text-gray-400'}`} />
                  <Sparkles className={`w-3.5 h-3.5 ${isDarkMode ? 'text-amber-400' : 'text-amber-500'} absolute -top-1 -right-1 animate-pulse`} />
                </div>
