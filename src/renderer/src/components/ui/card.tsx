...
export const CardHeader: React.FC<CardHeaderProps> = ({
  ...
  return (
    <div className={cn('flex items-center justify-between mb-4', className)}>
      <div>
        {title && <h3 className="text-lg font-semibold text-[var(--bready-text)]">{title}</h3>}
        {subtitle && <p className="mt-1 text-sm text-[var(--bready-text-muted)]">{subtitle}</p>}
      </div>
...
export const CardTitle: React.FC<CardTitleProps> = ({ children, className }) => {
  return (
    <h3 className={cn('text-lg font-semibold text-[var(--bready-text)]', className)}>{children}</h3>
  )
}
...
export const CardDescription: React.FC<CardDescriptionProps> = ({ children, className }) => {
  return <p className={cn('text-sm text-[var(--bready-text-muted)]', className)}>{children}</p>
}
