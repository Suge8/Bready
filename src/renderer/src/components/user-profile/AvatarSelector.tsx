...
export const AvatarSelector: React.FC<AvatarSelectorProps> = memo(
  ({ isOpen, onClose, currentAvatar, onSelect, isDarkMode = false }) => {
    const { t } = useI18n()
              ...
              <div className="grid grid-cols-4 gap-3">
                {PRESET_AVATARS.map((avatar, index) => (
                  <motion.button
                    ...
                    title={avatar.label}
                  >
                    <div className={cn('absolute inset-0 bg-gradient-to-br', avatar.gradient)} />
...
export const AvatarDisplay: React.FC<AvatarDisplayProps> = memo(
  ({ avatarId, size = 'md', className, onClick, showPulse = false }) => {
    const sizeClasses = {
      sm: 'w-8 h-8',
      md: 'w-12 h-12',
      lg: 'w-16 h-16',
