...
const PurchaseRecordItem: React.FC<{
  ...
  return (
    ...
    >
      ...
      >
        <CreditCard className={cn('w-5 h-5', isDarkMode ? 'text-gray-400' : 'text-gray-500')} />
      </div>

      {/* 主要信息 */}
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className={cn('font-medium truncate', isDarkMode ? 'text-white' : 'text-gray-900')}>
      ...
      <div className="text-right flex-shrink-0">
        <div className="flex items-center gap-2 justify-end">
          {hasDiscount && (
            <span
              className={cn('text-xs line-through', isDarkMode ? 'text-gray-600' : 'text-gray-400')}
            >
        ...
        {hasDiscount && (
          <div className={cn('text-xs mt-0.5', isDarkMode ? 'text-gray-500' : 'text-gray-400')}>
            {t('profile.history.saved') || '节省'}{' '}
            {formatCurrency(record.original_price - record.actual_price)}
          </div>
        )}
      </div>
    </motion.div>
