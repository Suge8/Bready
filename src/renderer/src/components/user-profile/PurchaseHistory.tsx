...
import { type PurchaseRecord } from '../../lib/api-client'
...
const PurchaseRecordItem: React.FC<{
  ...
  return (
    <motion.div
      ...
      className={cn(
        'flex items-center gap-2 px-2.5 py-1.5 rounded-lg border group',
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex items-center gap-2">
        ...
        <span
          className={cn(
            'flex items-center gap-0.5 text-[9px] px-1 py-0.5 rounded font-medium uppercase tracking-wide border bg-opacity-10 border-opacity-20 border-current',
            config.color,
            config.bgColor,
          )}
        >
      ...
      <div className="text-right flex-shrink-0">
        <div className="flex flex-col items-end gap-0">
          ...
          {record.expires_at && (
            <div className={cn('text-[9px]', isDarkMode ? 'text-gray-600' : 'text-gray-400')}>
...
export const PurchaseHistory: React.FC<PurchaseHistoryProps> = memo(({ isDarkMode = false }) => {
  ...
  if (records.length === 0) {
    return (
      ...
      >
        <CreditCard
          className={cn('w-8 h-8 mx-auto mb-2', isDarkMode ? 'text-gray-700' : 'text-gray-300')}
        />
    ...
    <div className="space-y-1">
      ...
      {hasMore && (
        <motion.button
          ...
          disabled={loading}
          className={cn(
            'w-full py-2 rounded-lg border text-xs font-medium flex items-center justify-center gap-1.5 mt-2',
