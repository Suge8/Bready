...
// 套餐列表骨架屏
export const PackageListSkeleton: React.FC<{ isDarkMode?: boolean; count?: number }> = memo(({ isDarkMode, count = 4 }) => (
    <div className="space-y-4">
        <SkeletonBase className="h-5 w-24" isDarkMode={isDarkMode} />
...
// 历史记录列表骨架屏
export const HistoryListSkeleton: React.FC<{ isDarkMode?: boolean; count?: number }> = memo(({ isDarkMode, count = 5 }) => (
    <div className="space-y-3">
        {Array.from({ length: count }).map((_, i) => (
            <motion.div
                key={i}
