...
const groupByDate = (records: InterviewUsageRecord[], locale: string) => {
    const groups: { [localDateKey: string]: { displayDate: string; records: InterviewUsageRecord[] } } = {}
...
const UsageRecordItem: React.FC<{
    ...
    const sessionTypeLabel = record.session_type === 'collaboration'
...
export const UsageHistory: React.FC<UsageHistoryProps> = memo(({
    isDarkMode = false
}) => {
    const { t, locale } = useI18n()
